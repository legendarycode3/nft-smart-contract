...
* **Safe NFT Minting**: NFTs are minted using the `_safeMint()` function, ensuring that tokens sent to smart contracts are transferred only if the receiving contract correctly implements the ERC-721 receiver interface. This helps prevent NFTs from becoming permanently
