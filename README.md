...
### MoodNft.sol
`MoodNft.sol` is a dynamic ERC-721 implementation that demonstrates how NFTs can evolve after minting. Instead of relying on external metadata stored on IPFS, the contract generates its metadata entirely on-chain by dynamically constructing JSON metadata
