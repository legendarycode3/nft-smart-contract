...
* **mintNft(string tokenUrl)**: Creates a new ERC-721 NFT and safely assigns ownership to the caller (`msg.sender`). During the minting process, the supplied metadata URI is stored and permanently associated with the newly created token before the NFT is minted.
