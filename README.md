...
```shell
  ...
  Mood = HAPPY
```
`mintNft()`, the contract generates a unique token ID using the token counter and safely mints the NFT to the caller. starts in the `HAPPY` state. The corresponding
